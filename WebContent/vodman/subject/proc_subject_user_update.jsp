...
<%
	...
	String user_idx = ""; // S - 설문 ,  E - 이벤트
	if(request.getParameter("user_idx") != null && request.getParameter("user_idx").length()>0){
		user_idx = request.getParameter("user_idx").replaceAll("<","").replaceAll(">","");
	} else {
		out.println("<SCRIPT LANGUAGE='JavaScript'>alert('사용자 정보가 없습니다.')self.close();</script>");
	}

	String question_count = request.getParameter("question_count").replaceAll("<","").replaceAll(">",""); 
...
//	String ans_order = ""; // textarea
	...
	int result = subjectbean.update_user(request);  // result 값으로user_idx 값을 받음 (사용자 정보 입력)
	if (result > 0) {
		// 기존 사용자 답변들 삭제 후 다시 저장
		int result4 = subjectbean.delete_info_user(user_idx);
	} else {
