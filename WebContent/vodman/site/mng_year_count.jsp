...
<%
	...
	v_list=stat.getYearStatTemp_Count(iDateSumYear, flag);

	float[] year_count = new float [13];
	
	if(v_list != null && v_list.size()>0){
		for (int j=0; j < v_list.size() ; j++) {
			try{
				year_count[Integer.parseInt(String.valueOf(((Vector)(v_list.elementAt(j))).elementAt(0)).substring(5,7))] =Integer.parseInt(String.valueOf(((Vector)(v_list.elementAt(j))).elementAt(1)));
			}
			catch(Exception ex)
			{
...
<body style="background-image:none;height:">
	...
	<table cellspacing="0" class="connection_view01" summary="연간페이징현황">
	<caption>연간페이징현황</caption>
	<colgroup>
		<col width="4%"/>
		<col width="4%"/>
		<col width="4%" span="22"/>
	...
	<thead>
		<tr>
			<th colspan="21" class="connection_view01_title"><b>연간페이징현황</b></th>
