...
MediaManager2014 mMgr2014 = MediaManager2014.getInstance();
...
///////////////////////////////////
	...
	Vector new_list_all = new Vector();
	new_list_all = mMgr2014.getMediaList_count(5);
	for (Enumeration e = new_list_all.elements(); e.hasMoreElements(); new_list_all.size()) {
		Hashtable ht = (Hashtable) e.nextElement();
		com.yundara.beans.BeanUtils.fill(oinfo, ht);
		//out.println(ht);
		if(oinfo.getCcode() != null &&  oinfo.getCcode().length()>3 && oinfo.getCcode().substring(0,3).equals("001")){
			new_list1.addElement(ht);
		}else if(oinfo.getCcode() != null &&  oinfo.getCcode().length()>3 && oinfo.getCcode().substring(0,3).equals("002")){
			new_list2.addElement(ht);
		}else if(oinfo.getCcode() != null &&  oinfo.getCcode().length()>3 && oinfo.getCcode().substring(0,3).equals("003")){
			new_list3.addElement(ht);
		}else if(oinfo.getCcode() != null &&  oinfo.getCcode().length()>3 && oinfo.getCcode().substring(0,3).equals("004")){
			new_list4.addElement(ht);
		}else if(oinfo.getCcode() != null &&  oinfo.getCcode().length()>3 && oinfo.getCcode().substring(0,3).equals("005")){
			new_list9.addElement(ht);
		}else if(oinfo.getCcode() != null &&  oinfo.getCcode().length()>3 && oinfo.getCcode().substring(0,3).equals("006")){
			new_list8.addElement(ht);
		}
	}
...
Vector vtBts_ontime = new Vector();
vtBts_ontime = Best_mgr.getBestTopSubList_order_union(2);
for (Enumeration e = vtBts_ontime.elements(); e.hasMoreElements(); vtBts_ontime.size()) {
	Hashtable ht = (Hashtable) e.nextElement();
	com.yundara.beans.BeanUtils.fill(btsBean, ht);
	//out.println(ht);
	if(btsBean.getBti_id() == 2 ){
		vtBts_1.addElement(ht);
	}else if(btsBean.getBti_id() == 3 ){
		vtBts_2.addElement(ht);
	}else if(btsBean.getBti_id() == 4 ){
		vtBts_3.addElement(ht);
	}
}
		...
		<div id="content">
			<div class="sectionTop">
				<div class="silver">
					...
					<%  if(live_v != null && live_v.size() > 0) {%>
						<span class="hot" id="today_live"><span>SUWON iTV LIVE</span>라이브 방송</span>
						<script type="text/javascript" >
							if (AC_FL_RunContent == 0) {
								...
								AC_FL_RunContent(
									...
									'FlashVars', 'userName=rtmp://<%=DirectoryNameManager.SV_LIVE_SERVER_IP%>/<%=channel%>&userId=<%=stream%>',
									'salign', ''
									); //end AC code
					...
					<span class="sub">
						<span class="title"><a>[<%=live_title %>] 생방송 진행 중!</a></span>
						<span class="subject"><%=live_rcontents %></span>
						<span class="more"><a href="javascript:live_open();" onclick='callToActionscript()' ><img src="../include/images/btn_blank.gif" alt="새창으로 보기"/></a>
						</span>
 					</span>	
				    <%} else {%>
				    <span class="hot" id="today_vod"><span>SUWON iTV TODAY</span>오늘의 주요 영상</span>
					<div class="vod">
							...
							<textarea id="player_" style="display:none;" rows="0" cols="0"/>
					          ...
					          <param name="initparams" value="Server=<%=DirectoryNameManager.SERVERNAME %>,Post=<%=oinfo.getOcode()%>,Auto_p=False"/>
						...
						<span class="more"><a  class="view_page" href="../video/video_player.jsp?ocode=<%=oinfo.getOcode()%>" onclick="StopSilverlight();"><img src="../include/images/btn_today_more.gif" alt="자세히보기"/></a>
...
<% 					
			if (vtBts_1 != null && vtBts_1.size() > 0) {
 			String list_img = "/2013/include/images/noimg_small.gif";
...
%>			
				...
				<div class="mLife">
					...
					<span class="more"><a href="<%=list_link%>" class="view_page"><img src="../include/images/btn_life_organ.gif" alt="편성표"/></a></span>
				</div>
...
<% 					
			if (vtBts_2 != null && vtBts_2.size() > 0) {
 			String list_img = "/2013/include/images/noimg.gif";
...
%>	
				...
				<div class="mNote">
					<h3><a href="/2013/board/board_list.jsp?board_id=16">제작현장</a></h3>
					...
					if (noticeVt2 != null && noticeVt2.size() > 0) {
					 ...
					 %>
					 	...
					 	<span class="img"><a class="view_page" href="../board/board_view.jsp?board_id=<%=bliBean.getBoard_id() %>&amp;list_id=<%=bliBean.getList_id()%>" onclick="StopSilverlight();"><img src="../board/img_2.jsp?board_id=<%=bliBean.getBoard_id() %>&amp;list_id=<%=bliBean.getList_id()%>" alt="제작현장"/></a></span>
						<span class="total">
				...
				<div class="mNotice">
					<h3><a href="../board/board_list.jsp?board_id=10">공지사항</a></h3>
				...
				<div id="popupzone">
					<div class="popup">
					<span class="count">
						<a href="#popupzone" id="pzPrev"><img src="../include/images/icon_left.gif" alt="팝업 이전" /></a>
						<strong class="currentCount">1</strong>&nbsp;/&nbsp;<%=popv_M.size() %>
						<a href="#popupzone" id="pzNext"><img src="../include/images/icon_right.gif" alt="팝업 다음" /></a>
					</span>
					<div class="btn_popup">
						<div>
							<a href="#popupZone" onclick="showHide('popupList');return false;"><img src="../include/images/popup_list.gif" alt="팝업 목록전체보기" /></a>
							<div id="popupList" style="display:none;">
								<ul>
								
								<% 
								int pop_size = 0;  // 6개까지만 보여줌 (text 목록)
						...
						<a href="#popupzone" id="pzPlay"><img src="../include/images/popup_stop.gif" alt="팝업 롤링정지" /></a>
					</div>
					<dl>
		...
		<div class="mAside"> 
			<iframe title="포토뱅크" id="photo_bank" name="photo_bank" src="http://photo.suwon.go.kr/photo_bank_withustech.asp" scrolling="no" width="231" height="250" marginwidth="0" frameborder="0" framespacing="0" ></iframe>
			<div class="hotnews">
				<h3>HOT 뉴스</h3>
				<ul>
<% 					
...
<%							
			...
			if (cnt < 4) {
				for (int c = cnt ; c < 4; c++) {
					%>
					<li class="small">
					<span class="img"><span class="playSmall2"></span><img src="/2013/include/images/noimg.gif" alt="등록된 정보가 업습니다."/></span>
					</li>
					<% 
				}
			}
...
%>				
			...
			<div class="live">
				<h3><a href="../live/live_list.jsp">생방송</a></h3>
			...
			<div class="promote">
				<h3><a href="../video/video_list.jsp?ccode=005000000000">홍보영상</a></h3>
