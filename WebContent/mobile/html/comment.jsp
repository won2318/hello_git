...
//  메모 읽기
   ...
   if(memoVt == null || (memoVt != null && memoVt.size() <= 0)){
	...
	<h5>댓글 <strong><%=memo_size %></strong></h5>
	...
	<input type="hidden" name="muid" value="" />
		...
		<dd><input type="text" name="wnick_name" value="" id="wnick_name" title="닉네임" onclick="javascript:document.getElementById('wnick_name').value='';"/></dd>
		</dl>
		<dl>
		<dt><label for="pwd">비밀번호</label></dt>
		<dd><input type="password" name="pwd" value=""  id="pwd" title="비밀번호"  onclick="javascript:document.getElementById('pwd').value='';"/></dd>
		</dl>
		<dl>
		<dt><label for="chk_word">확인문자열</label></dt>
		<dd><input type="text" name="chk_word" value="" id="chk_word" size="10" title="확인 문자열"  />
			<span class="chk1"><%=iRandomNum%></span> * 좌측 숫자 입력
		</dd>
		</dl>
		<div class="input_area">
			<textarea id="comment_write" name="comment" wrap="hard"  required="required" ></textarea>
			<input type="submit" alt="확인" class="img" value="확인" onclick="comment_action()"/>
		</div>
		</form>
	</div>
...
%>
		 	    ...
		 	    <dl>
					<dt class="name"><%=memoBean.getWnick_name()%> <span class="day"><%=memoBean.getWdate() %></span></dt>
					<dd class="subject">
						<span class="content"><%=chb.getContent_2(memoBean.getComment(),"true")%></span>
						<span class="del">
							<a href="javascript:deleteChk('<%=memoBean.getMuid()%>');">Del</a>
						</span>
					</dd>
				</dl>
...
<%
 ...
 <% 
	...
	<div class="paginate">
		...
		<a href="javascript:page_go(<%=((mPageBean.getCurrentBlock()-2)*mPageBean.getPagePerBlock()+1)%>)" class="back1" title="이전페이지"><img src="../include/images/icon_back1.gif" alt="이전페이지"></a>
		<%
		...
		%>
					<a href="javascript:page_go(<%= mPageBean.getCurrentBlock()*mPageBean.getPagePerBlock()+ 1%>)" class="next1" title="다음페이지"><img src="../include/images/icon_next1.gif" alt="다음페이지"></a>
		<%
				}
		%>
