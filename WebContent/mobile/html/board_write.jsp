...
<!--  이벤트 참여 개인정보 수집 -->	
				<div class="privacy">
						<p> 수원시는 수원iTV 모바일 페이지 활성화를 위해 <strong>'영상 보고, 도넛 먹자!' 이벤트 진행</strong>을 위해
						...
						<div class="privacy_inner">
							<ul>
								...
								<ul>
									<li>(1) 수집 및 이용 항목 : 성명, 전화번호, 생년월일 앞자리</li>
									<li>(2) 수집 및 이용 목적 : 이벤트 진행 및 당첨자에게 기프티콘 발송</li>
									<li>(3) 보유 기간 : 2017. 4. 24.~ 2017. 5. 8.(월), 15일</li>
								</ul>
								</li>
								<li> <span style="color:#00a1e0;font-weight:600">* 14세 미만은 참가 불가능합니다.</span><br/> 
...
<%} %>				
				...
				<dl>
					...
					<dd><input type="text" name="list_title" value="" id="list_title"  title="제목" class="wd100"/>
					 <% if (board_id == 22) { %>
				...
				<dl>
					...
					<%if (board_id == 22 ) { %>
					<dd><%=vod_id %><input type="hidden" name="list_name" value="<%=vod_name %>" id="list_name" title="작성자" class="wd100"/></dd>
					<%} else { %>
					<dd><input type="text" name="list_name" value="<%=vod_name %>" id="list_name" title="작성자" class="wd100"/></dd>
					<%} %>
				 ...
				 <%if (board_id == 22 ){ %>
				<dl>
					<dt><label for="image_text8">구분</label></dt>
					<dd>
					 <input type="radio" name="image_text8" id="image_text8" value="V" checked="checked"/><label for="image_text8">영상(수원iTV)</label>
					&nbsp;&nbsp;<input type="radio" name="image_text82"  id="image_text82" value="N" /><label for="image_text82">기사(e수원뉴스)</label>
					</dd>
				</dl>
				<%} %>
				...
				<dl>
					...
					<dd><textarea name="list_contents" style="line-height: 170%;"  rows="15" id="list_contents" title="내용" class="wd100">
					<%
...
<%
					//시민모니터링단 게시판만 기본으로 제공
					if (board_id == 22 ) { %>
					<div style="display:block; padding-top:5px; width:100%;">
※인텐시브 인정기준
...
<P>※ 인센티브 제공<br/>&nbsp;- 보상금은 <strong>예산의 범위 내</strong>에서 집행 / 예산 소진시에는 자원봉사활동시간으로 지급<BR></P></div>
				...
				<dl>
					...
					<dd><input type="file" id="file" name="list_data_file" size="75" value="" title="첨부파일" class="wd100"/></dd>
					
				</dl>
				 <%} %>
				  <%if(board_image_flag.equals("t")){  // 모바일은 이미지 하나만%>
				 <dl>
					<dt><label for="list_image_file">이미지</label></dt>
				...
				<dl>
					...
					<dd><textarea name="image_text" id="image_text"  rows="3" class="wd100"></textarea></dd>
				</dl>
				 <dl>
					<dt><label for="list_image_file2">이미지</label></dt>
				...
				<dl>
					...
					<dd><textarea name="image_text2" id="image_text2"  rows="3" class="wd100"></textarea></dd>
				</dl>
				 <dl>
					<dt><label for="list_image_file3">이미지</label></dt>
				...
				<dl>
					...
					<dd><textarea name="image_text3" id="image_text3"  rows="3" class="wd100"></textarea></dd>
				</dl>				 
				<%} %>
				<dl>
					<dt><label for="chk_word">확인문자열</label></dt>
					<dd><input type="text" name="chk_word" value="" id="chk_word" size="10" title="확인 문자열"   class="wordcheck"/> 
					<i><%=iRandomNum %></i>* 좌측 숫자 입력</dd>
				</dl>
				<% if (board_auth_write == 0) { %>
				<dl>
					<dt><label for="password">비밀번호</label></dt>
					<dd><input type="password" name="list_passwd" value="" id="password" size="10" title="비밀번호"/></dd>
				</dl>
				<% } %>
