	...
	if(vt != null && vt.size() > 0){
		...
		try{
			for(int i = pageBean.getStartRecord()-1; i < pageBean.getEndRecord(); i++) {
				...
				String osimple = TextUtil.replace(oinfo.getContent_simple(), "'", "\\'");
...
%>
						...
						<tr>
							...
							<td rowspan="2"><a href="javascript:set_ocode('<%=request.getParameter("val")%>', '<%= outTitle %>','<%=osimple%>','<%=listImage%>','<%=oinfo.getOcode()%>');" title="<%=otitle%>"><img src="<%=listImage%>" alt="이미지" class="img_style02"/></a></td>
							<td class="main_list_cate"><%=ctitle%></td>
						</tr>
						<tr>
							<td class="main_list_title"><a href="javascript:set_ocode('<%=request.getParameter("val")%>', '<%= outTitle %>','<%=osimple%>','<%=listImage%>','<%=oinfo.getOcode()%>')" title="<%=otitle%>"><%=otitle%></a></td>
						</tr>
