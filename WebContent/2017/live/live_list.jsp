...
<script language="JavaScript" type="text/JavaScript">
	...
	function next(year) {
		...
		<section id="body">
			<div id="container_out">
				<div id="container_inner">
					...
					<table class="schedule" cellspacing="0" summary="생방송일정으로 방송일, 방송시간, 방송제목, 다시보기를 제공합니다.">
 	...
 	if (s_date <= this_date && this_date <= e_date) {
 					out.print("<a href='javascript:live_open()' class='on-air'>ON-AIR</a>");
 				}
 %>
										<%
										...
										%>
										<a  
										href="../video/video_list.jsp?ccode=006001000000&ocode=<%=linfo.getOcode()%>">1</a> <%
 	}
 				if (linfo.getOcode2() != null
 						&& linfo.getOcode2().length() > 0) {
 %>
										<a  
										href="../video/video_list.jsp?ccode=006001000000&ocode=<%=linfo.getOcode2()%>">2</a> <%
 	}
 				if (linfo.getOcode3() != null
 						&& linfo.getOcode3().length() > 0) {
 %>
										<a  
										href="../video/video_list.jsp?ccode=006001000000&ocode=<%=linfo.getOcode3()%>">3</a> <%
 	}
 %>
									</span>
								</dd>
