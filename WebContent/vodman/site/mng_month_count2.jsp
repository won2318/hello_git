...
<body style="background-image:none;height:">
	...
	<table cellspacing="0" class="connection_view01" summary="월간시청현황">
	<caption>월간시청현황</caption>
	<colgroup>
		<col width="5%"/>
		<col width="3%"/>
		<col width="3%" span="30"/>
	...
	<thead>
		<tr>
			<th colspan="27" class="connection_view01_title"><b>월간시청현황</b></th>
...
if(request.getParameter("flag") != null && request.getParameter("flag").length()>0 && !request.getParameter("flag").equals("null")){
	...
	v_list=stat.getMonthStatCount(iDateSumMon,flag);
 
	float[] month_count = new float [maxday];
	try{
		if(v_list != null && v_list.size()>0){
			for (int j=0; j < v_list.size() ; j++) {
				month_count[j] =Integer.parseInt(String.valueOf(((Vector)(v_list.elementAt(j))).elementAt(0)));			
			}
		}
	}catch(Exception ex){
	...
	{
	  val=Math.round(month_count[i-1]);

		try{
			 
			if(val>max_height)
				max_height=Math.round(month_count[i-1]);
	...
	for(int i=1; i<maxday; i++)
	{
		...
		int height=0;
		try{
			cnt=Math.round(month_count[i-1]);
