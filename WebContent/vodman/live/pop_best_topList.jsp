...
//	request.setCharacterEncoding("EUC-KR");
    ...
    String target2 = "";
     if(request.getParameter("target") != null)
    	 target = request.getParameter("target").replaceAll("<","").replaceAll(">","");
     if(request.getParameter("target2") != null)
    	 target2 = request.getParameter("target2").replaceAll("<","").replaceAll(">","");
...
<script language="javascript">
	...
	function set_ocode(ocode, otitle) {
		target = '<%=target%>';
		target2 = '<%=target2%>';
		window.opener.document.getElementById(target).value = ocode;			
		window.opener.document.getElementById(target2).value = otitle;
 			window.close();
		}
</script>
<script language="javascript">
