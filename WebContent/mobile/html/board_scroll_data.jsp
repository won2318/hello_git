...
// 		              "</script>");
 ...
 if (flag != null && flag.equals("P")) {  // 포토 게시판 목록
 		...
 		if(vt_list != null && vt_list.size() > 0 && pageBean != null && pageBean.getEndRecord() > 0)
		{
			...
			<li class="pin">	
						<a href="<%if ( list_security != null && list_security.equals("Y")  && ( list_name == null || vod_name == null ||  !vod_name.equals(list_name)) ) {%>javascript:alert('비공개 글입니다.');<%}else{ %>./board_view.jsp?list_id=<%=list_id%>&amp;board_id=<%=board_id %><%} %>">
			...
			<%
						if(board_hidden_flag != null && board_hidden_flag.equals("t")){  // 비밀글(숨김) 게시판
							if ( board_auth_write == 0){
								...
								<%	} %><%=list_title%><span class="secret"><img src="../include/images/icon_secret.png" alt="비밀"/></span>
						...
						}else if(board_security_flag != null && board_security_flag.equals("t")){ //비밀글 사용 게시판
							if ( board_auth_write == 0){
						...
						<%
							...
							<%		} %>
									<img src="../include/images/icon_reply_title.gif" width="39" height="18" alt="답글"/>
							<%	} %>
							<%if (board_id == 22 && image_text8 != null && image_text8.equals("V") ) {out.print("영상(수원iTV) ");} %>
							<%=list_title%><span class="update"><strong><%=list_name %></strong><%=list_date %></span></a>
						<%
						}
						%> 
