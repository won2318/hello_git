...
//	}
	...
	function vod_view(ocode){
		...
		window.open(url,"700k_player","width=915,height=790,scrollbars=yes,status=no");
 ...
 <form name='subject_user' method='post' action="proc_subject_user.jsp">
	...
	//out.println(vt1);
			...
			{ // for (vt1)
			   ...
			   <!-- ///////////설문 (다중반복)/////////////  -->
					...
					%>			
						...
						<ul>
								<textarea name="<%=question_idx%>" cols="85" rows="5" id="subject<%=question_idx%>" title="내용"></textarea> 
			...
			<tr>
				...
				<td><input type="radio" name="<%=question_idx%>_<%=ans_num%>" value="<%=k+1%>"  title="<%=question_idx%>_<%=ans_num%>" /></td>
				<%	}
				}%>
						...
						<%	}else if (question_option.equals("3")){ // 다중선택 제한
							%> 
									<li>
										(<%=j+1%>) <input name="<%=question_idx%>" id="<%=question_idx%><%=j+1%>" type="checkbox" onClick='checkCheckbox(this.form,this,<%=question_etc %>)' value="<%=ans_num%>" />
										<label for="<%=question_idx%><%=j+1%>"><%=ans_content%></label>&nbsp;<% if (ans_order != null && ans_order.equals(ans_num)) {  out.println("&nbsp;&nbsp;<span class='inputType'><input type='text' size='35'  title='기타'  name='"+question_idx+"_order_"+ans_num+"' class='inputType' /></span>"); }  %>
								  	</li>
						...
						<%	}else if (question_option.equals("2")){ // 다중선택 무제한
							%>
									...
									<li>
										(<%=j+1%>) <input name="<%=question_idx%>"  id="<%=question_idx%><%=j+1%>" type="checkbox" value="<%=ans_num%>" />
										<label for="<%=question_idx%><%=j+1%>"><%=ans_content%></label>&nbsp;<% if (ans_order != null && ans_order.equals(ans_num)) { out.println("&nbsp;&nbsp;<span class='inputType'><input type='text' size='35' title='기타' name='"+question_idx+"_order_"+ans_num+"' class='inputType' /></span>"); }  %>
								  	</li>
						...
						<%	}else if (question_option.equals("1")){ %>
									...
									<li>
										(<%=j+1%>) <input type="radio" name="<%=question_idx%>"  id="<%=question_idx%><%=j+1%>" value="<%=ans_num%>" />
										<label for="<%=question_idx%><%=j+1%>"><%=ans_content%></label>&nbsp;<% if (ans_order != null && ans_order.equals(ans_num) ) { if ( sub_flag.equals("S")) {  out.println("&nbsp;&nbsp;<span class='inputType'><input type='text' size='35'  title='기타' name='"+question_idx+"_order_"+ans_num+"' class='inputType' /></span>"); } } %>
								 	</li>
						...
						%>
								...
								<li>
									<span class="img">(<%=j+1%>) <input name="<%=question_idx%>"  id="<%=question_idx%><%=j+1%>" type="checkbox" onClick='checkCheckbox(this.form,this,<%=question_etc %>)' value="<%=ans_num%>" />
									...
									<span class="sub">
										...
										&nbsp;<% if (ans_order != null && ans_order.equals(ans_num) ) { if ( sub_flag.equals("S")) { out.println("&nbsp;&nbsp;<span class='inputType'><input type='text' size='35'  title='기타' name='"+question_idx+"_order_"+ans_num+"' id='"+question_idx+"_order_"+ans_num+"' class='inputType' /></span>"); } } %>
									</span>
								</li>
		...
		%>
				...
				<p class="moongu sem"><a href="http://tv.suwon.go.kr" target="_blank"><img src="../include/images/main/logo.png" alt="수원iTV" width="90" height="45" /></a> <span>끝까지 설문에 응해주셔서 감사합니다. <br/>귀하의 소중한 의견을 반영하여 보다 나은 수원시 인터넷방송 만들기 위해 최선을 다하겠습니다.</span></p>
