...
if (request.getParameter("ccategory4") != null) {
    ...
    String num = "";
     if(request.getParameter("val") != null)
        num = request.getParameter("val").replaceAll("<","").replaceAll(">","");
...
<script language="javascript">
	...
	function set_ocode( num, otitle,content,img, ocode) {

        var ans_content = "ans_content_"+num;
        var ans_etc = "ans_etc_"+num;
        var ans_img = "ans_img_"+num;
        var ans_hot7_ocode = "ans_hot7_ocode_"+num;
        window.opener.document.getElementById(ans_content).value=otitle; 
    	window.opener.document.getElementById(ans_etc).value=content;
    	window.opener.document.getElementById(ans_img).value=img; 
    	window.opener.document.getElementById(ans_hot7_ocode).value=ocode; 
    	window.close();
...
function check_form(){
	 if (document.hot7_upload.img_upload.value == '') {
		 alert("이미지를 선택하세요!");
	 } else if (document.hot7_upload.title.value == '') {
		 alert("제목을 입력하세요!");
	 } else if(document.hot7_upload.content.value == ''){
		alert("간략설명을 입력하세요!"); 
	 } else {	 
	 	document.hot7_upload.submit();
	 }
	...
	<div id="popup_bg_cen">
		<table cellspacing="0" class="log_list" summary="동영상검색">
			...
			<colgroup>
				...
				<tbody>
				<form name="search" method="post" action="pop_hot7.jsp">
	...
	if(vt != null && vt.size() > 0){
		...
		try{
			for(int i = pageBean.getStartRecord()-1; i < pageBean.getEndRecord(); i++) {
				...
				String osimple = oinfo.getContent_simple();
				osimple = osimple.replaceAll("&#39;", "").replaceAll("\"","").replaceAll("\n","").replaceAll("\r","").replaceAll("\n\r",""); 
...
//				DirectoryNameManager Dmanager = new DirectoryNameManager();
				...
				listImage =SilverLightPath + "/"+oinfo.getSubfolder() +"/thumbnail/_small/"+oinfo.getModelimage();
...
%> 
						<tr>
							<td rowspan="2" class="pa_left"><input type="checkbox" id="check_<%=String.valueOf(oinfo.getOcode())%>" name="check_<%=String.valueOf(oinfo.getOcode())%>" value="<%=String.valueOf(oinfo.getOcode())%>"  onclick="set_ocode('<%=request.getParameter("val")%>', '<%= outTitle %>','<%=osimple%>','<%=listImage%>','<%=oinfo.getOcode()%>');" /></td>
							<td rowspan="2"><a href="javascript:set_ocode('<%=request.getParameter("val")%>', '<%= outTitle %>','<%=osimple%>','<%=listImage%>','<%=oinfo.getOcode()%>');" title="<%=outTitle%>"><img src="<%=listImage%>" alt="이미지" class="img_style02"/></a></td>
							<td class="main_list_cate"><%=ctitle%></td>
						</tr>
						<tr>
							<td class="main_list_title"><a href="javascript:set_ocode('<%=request.getParameter("val")%>', '<%= outTitle %>','<%=osimple%>','<%=listImage%>','<%=oinfo.getOcode()%>')" title="<%=outTitle%>"><%=otitle%></a></td>
						</tr> 
<%
			}
		}catch(Exception e) {
...
<%
			...
			<%} %>
				...
				<p style="font-size:14px; text-align: center; font-weight: bold; padding: 10px; color:#252525;">이미지등록</p>
				<form name="hot7_upload" method="post"  enctype="multipart/form-data" action="proc_hot7_upload.jsp?num=<%=request.getParameter("val")%>">
				<table cellspacing="0" class="log_list" summary="동영상검색">
				    	<colgroup>
							<col width="15%" class="back_f7"/>
							<col/>
						...
						<tbody>
					    <tr>
						  <th  class="bor_bottom01">이미지</th>
						  <td class="bor_bottom01 pa_left"><input type="file"  style="width:250px;"  name="img_upload" value="" /></td>
						</tr>
						 <tr >
						  <th class="bor_bottom01">제목</th>
						  <td  class="bor_bottom01 pa_left"><input type="text" name="title"  style="width:350px;" class="input01" value="" /></td>
						</tr>
						 <tr >
						  <th class="bor_bottom01" >간략설명</th>
						  <td   class="bor_bottom01 pa_left"><input type="text" name="content" style="width:350px;" class="input01"  value="" /></td>
						</tr>
						<tr >
							<td class="bor_bottom01" align="center" colspan="2"><a href="javascript:check_form()"><img src="/vodman/include/images/but_save.gif" alt="추가" border="0"/></a></td>
						</tr>
						</tbody>
					</table> 
				</form>
