...
<script language='javascript'>
	...
	function checkInverse() {
		...
		<!-- 컨텐츠 -->
		<div id="contents">
			<h3><span>영상 댓글관리</span></h3>
			<p class="location">관리자페이지 &gt; 영상관리 &gt; 영상 댓글관리 &gt; <span>영상 댓글</span></p>
			...
			<div id="content">
				...
				<tbody>
				 <!-- list start-->
                            			...
                            			Vector menu_vt = mgr.getMenuListALL2("A");
                            			MenuInfoBean menu_info = new MenuInfoBean();
											...
											if(v_bl != null && v_bl.size() >0){
												...
												for(int i = mPageBean.getStartRecord()-1; i < mPageBean.getEndRecord() && (list<v_bl.size()); i++, list++) {
													  ...
													  int menu_idx = 2;
													  String tmp_menucode = "";
													for(Enumeration e2 = menu_vt.elements(); e2.hasMoreElements();) {
				                            			com.yundara.beans.BeanUtils.fill(menu_info, (Hashtable)e2.nextElement());
				                            			if(menu_info.getMurl().indexOf(ccode) != -1){
				                            				//메뉴 인덱스와
				                            				//ccode를 이용해서
				                            				//mcode를 결정한다.

				                            				tmp_menucode = "07" +( menu_idx>9?""+menu_idx:"0"+menu_idx );
				                            				break;
				                            			}
				                            			menu_idx ++;
				                            		}
					...
					<tr class="height_25 font_127 bor_bottom01"  onMouseOver="this.style.backgroundColor='#F6F6F6'" onMouseOut="this.style.backgroundColor=''">
						...
						%>
						<a href="frm_boardListView.jsp?board_id=<%=board_id%>&list_id=<%=list_id%>" title="게시물 확인">
						<%}else if (flag != null && flag.equals("M")){ %>
						<a href="../vod_aod/frm_updateContent.jsp?ocode=<%=list_id%>&ccode=<%=ccode%>&mcode=<%=tmp_menucode%>" title="게시물 확인">
						<%} %>
						<%=list_title%></a></td>
