...
int board_auth_write = 0;
	...
	int event_seq = NumberUtils.toInt(request.getParameter("event_seq"), 0);
 		...
 		if (board_auth_write == 0 && request.getParameter("pwd") != null &&  SEEDUtil.getEncrypt(request.getParameter("pwd")).trim().equals(list_pwd)) {
	 	} else if (user_key != null && user_key.equals(list_pwd)) {
		} else {
			out.println("<SCRIPT LANGUAGE='JavaScript'>");
			out.println("alert('정보가 일치 하지 않습니다. ')"); 
			out.println("history.go(-1)");
			out.println("</SCRIPT>");
		}
...
<% if (board_auth_write == 0) { %>
		...
		f.action="proc_boardListUpdate.jsp?board_id=<%=board_id%>&page=<%=pg%>&list_id=<%=list_id%>&pwd="+document.getElementById("password").value;
<%} else {%>
...
<!-- 				</div>//생방송안내(생방송이 있을때만 표출:mLive -->
				...
				<dl>
					...
					<dd><input type="text" name="list_title" value="<%=list_title %>" id="list_title"  title="제목" class="wd100"/></dd>
				</dl>
				<dl>
				...
				<%if (event_seq > 0 ) { %>
					<dl>
						...
						<dd><input type="text" name="user_address" value="" id="user_address" title="주소" class="wd100"/></dd>
					</dl>
					<dl>
						<dt><label for="user_tel">연락처</label></dt>
						<dd><input type="tel" name="user_tel" value="" id="user_tel" maxlength="14" title="연락처"/>  
						</dd>
					</dl>
				<%}%> 
				...
				<dl>
					...
					<dd><textarea name="list_contents" rows="15" id="list_contents" title="내용" class="wd100"><%=list_contents %></textarea></dd>
				</dl>
				...
				<dl>
					...
					<dd><textarea name="image_text" id="image_text"  rows="3" class="wd100"><%=image_text %></textarea></dd>
				</dl>
				 <dl>
					<dt><label for="list_image_file">이미지2</label></dt>
				...
				<dl>
					<dt><label for="image_text">설명</label></dt>
					<dd><textarea name="image_text2" id="image_text2"  rows="3" class="wd100"><%=image_text2 %></textarea></dd>
				</dl>
				 <dl>
					<dt><label for="list_image_file">이미지3</label></dt>
				...
				<dl>
					<dt><label for="image_text">설명</label></dt>
					<dd><textarea name="image_text3" id="image_text3"  rows="3" class="wd100"><%=image_text3 %></textarea></dd>
				</dl>
				...
				<% } %>
					...
					<div class="btn1 btn-03">
						...
						<a href="javascript:insertListBoard();" >수정</a>
