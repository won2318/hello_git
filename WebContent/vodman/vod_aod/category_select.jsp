...
<%
	...
	String ctype = request.getParameter("ctype").replaceAll("<","").replaceAll(">","");
	String info = request.getParameter("info").replaceAll("<","").replaceAll(">","");
	String category = request.getParameter("category").replaceAll("<","").replaceAll(">","");
	...
	if(info.equals("A") || category != null && category.length()>0){
		if (category == null || category.equals("")||category.equals("null")) {
			vList = cmgr.getCategoryListALL2(ctype, info);
		} else {
			vList = cmgr.getCategoryListALL2(ctype, info, category);
		}
	}
