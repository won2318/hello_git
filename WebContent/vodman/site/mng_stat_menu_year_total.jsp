...
<%
	...
	try{	
		if(request.getParameter("ydate") != null && request.getParameter("ydate").length() > 0){
			ydate = request.getParameter("ydate").replaceAll("<","").replaceAll(">","");
		}
	}catch(Exception e){
		ydate = null;
	}
...
<script>
	...
	function Search(){
		location.href='mng_stat_menu_year_total.jsp?mcode=<%=mcode%>&ydate='+document.frmpop.ytime.value;
	}
	
	function ExcelDown(){
		location.href='mng_stat_menu_year_total_excel2.jsp?mcode=<%=mcode%>&ydate='+document.frmpop.ytime.value;
	}
</script>
<%@ include file="/vodman/site/site_left.jsp"%>
		...
		<div id="contents">
			...
			<div id="content">
				...
				<ul class="s_tab01_bg">
				<li><a href="mng_stat_menu_total.jsp?mcode=<%=mcode%>&flag=T" title="Today" <%if(flag.equals("T")){out.println("class='visible'");}%>>일 통계</a></li>
				<li><a href="mng_stat_menu_month_total.jsp?mcode=<%=mcode%>&flag=M" title="Month" <%if(flag.equals("M")){out.println("class='visible'");}%>>월 통계</a></li>
				<li><a href="mng_stat_menu_year_total.jsp?mcode=<%=mcode%>&flag=Y" title="Year" <%if(flag.equals("Y")){out.println("class='visible'");}%>>년 통계</a></li>
				</ul>
				<br><br>
