...
$(document).ready( function(){
	 $.get('/2017/comment/chat_room.jsp', {'ocode' : '<%=rcode%>' }, function(data){
	...
	if(filterIng(document.getElementById('comment').value, "comment") == false){
		...
		if (document.getElementById('wnick_name').value == '') {
	     ...
	     var bodyContent = $.ajax({
			    url: "/2017/comment/chat_room.jsp",  //<- 이동할 주소 
			    global: false, //<- 
			    type: "POST",  //<-  보낼때 타입 
...
$('#chatList').load(
		 $.post('/2017/comment/chat_room.jsp', {'ocode' : '<%=rcode%>','wnick_name':document.getElementById('wnick_name').value }, function(data){
...
<div id="pWrapNormal">
	<!-- container::메인컨텐츠 -->
	<div id="pLogo">
		<span class="close"><a href="javascript:self.close();"><img src="../include/images/btn_view_close.gif" alt="닫기"/></a></span> 
 	</div>
...
<%-- 						<video width="640" height="400" controls="controls" src="http://<%=temp_server_name%>/<%=channel%>/playlist.m3u8"> --%>
<!--    						</video> -->    <!-- 스트리밍 입력 누가한건지??   -->
				</div> 
			</div>
...
<!-- 							<li><input type="image" src="../include/images/btn_chat_nick_pw_ok.gif" alt="확인" class="img"/></li> -->
						...
						<div class="input_wrap">
							...
							<input type="submit" alt="입력" class="img" value="입력"/>
						</div>
					</form>
					</div> 
