...
<%
	String paramOcode = null;
if (request.getParameter("ocode") != null) {
	paramOcode=request.getParameter("ocode").replaceAll("<","").replaceAll(">","");
	paramOcode = StringUtils.defaultString(paramOcode,"");
}
...
<script language='javascript'>
	...
	function checkInverse() {
		...
		<!-- 컨텐츠 -->
		<div id="contents">
			<h3><span>생방송 댓글관리</span></h3>
			<p class="location">관리자페이지 &gt; 생방송관리 &gt; 생방송 정보 &gt; <span>생방송 댓글</span></p>
				...
				<tbody>
				 <!-- list start-->
					...
					<td class="bor_bottom01"><input type="checkbox" name="v_chk" value="<%=binfo.getMuid()%>"/>&nbsp;</td>
						...
						<a href="/vodman/live/frm_Live_Update.jsp?mcode=0603&rcode=<%=list_id%>" title="게시물 확인">
						<%} %>
						<%=list_title%></a></td>
