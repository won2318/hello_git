...
<%
	...
	Vector vt =  (Vector)mgr.getMenuListALL3_0(today, "W");
	Vector vt1 = (Vector)mgr.getMenuListALL3_2(today, "W");
	
	Vector Mvt =  (Vector)mgr.getMenuListALL3_0(today, "M");
	Vector Mvt1 = (Vector)mgr.getMenuListALL3_2(today, "M");
...
// 	String[] titles = { "1","2","3","" };
	...
	//    else wb = new XSSFWorkbook();
		...
		titleCell.setCellValue("메뉴 클릭 년 통계");
		...
		for (int i = 1; i < 12+1; i++) {
			...
			headerCell = headerRow.createCell(12+1);
			...
			if(vt != null && vt.size()>0) {
				for(int i=0; i<vt.size(); i++) {
						...
						for(int g=0; g<12; g++){
							...
							row.getCell(12+1).setCellValue(csumM);
						}
				...
				for(int i=0; i<Mvt.size(); i++) {
					...
					for(int g=0; g<12; g++){
						...
						row.getCell(12+1).setCellValue(McsumM);
					}
				...
				row.getCell(12+1).setCellValue(Totalsum);
				...
				row1.getCell(12+1).setCellValue(MTotalsum);
