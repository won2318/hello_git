...
%><%
	...
	if(request.getParameter("ctype") != null) {
		...
		Vector vt =  mgr.getOVODMemberList_Excel( rstime, retime, ocode,  user_id, "",  ctype,  dept,  grade );
		if(vt == null || vt.size() <=0){
			return;
		}
%><%
	String[] titles = {
	            "번호",	"아이디", "생방송 제목", "시청일", "아이피"
	    };
...
//
	...
	//    else wb = new XSSFWorkbook();
		...
		Sheet sheet = wb.createSheet("생방송");
		...
		try{
			if(vt != null && vt.size()>0) {
				...
				for(int i=0; i<vt.size(); i++) {
					...
					row.getCell(3).setCellValue((String)regDate);
					row.getCell(4).setCellValue((String)ip);
		...
		String file = DirectoryNameManager.VODROOT+"/dbtemp/live.xls";
		String file_name = "live.xls";
