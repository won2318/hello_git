			...
			<div class="sectionTop">
				<div class="silver">
					...
					<div class="vod">
						<div id="SilverlightControlHost" class="silverlightHost" >
							<iframe title="실버라이트 동영상 재생 창" id="bestVod" name="bestVod" src="/silverPlayer.jsp?isplay=false&amp;width=521&amp;height=339&amp;ocode=<%=oinfo.getOcode() %>" scrolling="no" width="521" height="339" marginwidth="0" frameborder="0"></iframe>
						</div> 
					...
					<span class="sub">
						...
						<span class="more"><a href="#" onclick="showHide('vodSubject');return false;"><img src="../include/images/btn_today_more.gif" alt="자세히보기"/></a>
							<div id="vodSubject" style="display:none;">
								<!-- 오늘의 주요영상 정보 -->
								<div id="vodSubjectClose">
									<span class="close"><a href="#" onclick="showHide('vodSubject');return false;"><img src="../include/images/btn_view_close.gif" alt="닫기" width="37" height="37"/></a></span>
								</div>
								<div id="vodSubjectNormal">
									<h3 class="vodSubjectTitle"><%=oinfo.getTitle() %></h3>
									<span class="vodSubjectTime">등록일 : <%=oinfo.getMk_date() %>&nbsp;&nbsp;|&nbsp;&nbsp;방송시간 : <%=oinfo.getPlaytime() %></span>
									<div class="vodSubjectSubject">
										<%=oinfo.getDescription() %>
									</div>
									<span class="pRecomIcon"><a href="javascript:point_go('<%=oinfo.getOcode()%>');"><img src="../include/images/btn_like.gif" alt="좋아요 Like!"/></a></span>
								</div>
							</div>
						</span>
 					</span>
				...
				<div class="NewTab list1 jx">
					...
					%>
								...
								<li>
									<span class="img"><a class="view_page" href="../video/video_player.jsp?ccode=<%=oinfo.getCcode()%>&amp;ocode=<%=oinfo.getOcode()%>"><span class="playSmall"></span><img src="<%=imgTmp%>" alt="<%=oinfo.getTitle()%>"/></a></span>
									<span class="total">
										<span class="cate"><a class="view_page" href="../video/video_player.jsp?ccode=<%=oinfo.getCcode()%>&amp;ocode=<%=oinfo.getOcode()%>"><%=oinfo.getCtitle()%></a></span>
										<a class="view_page" href="../video/video_player.jsp?ccode=<%=oinfo.getCcode()%>&amp;ocode=<%=oinfo.getOcode()%>"><%=oinfo.getTitle()%></a>
									</span>
								</li>
							...
							if (new_list5 != null && new_list5.size() > 0) {
 							try {
								...
								for (Enumeration e = new_list5.elements(); e.hasMoreElements(); i++) {
									com.yundara.beans.BeanUtils.fill(oinfo,(Hashtable) e.nextElement());
					%> 	 
								<li><span class="time"><img src="../include/images/icon_best<%=i %>.gif" alt="<%=i %>"/></span> <a class="view_page" href="../video/video_player.jsp?ccode=<%=oinfo.getCcode()%>&amp;ocode=<%=oinfo.getOcode()%>"><strong>[<%=oinfo.getCtitle() %>]</strong> <%=oinfo.getTitle()%></a> </li>
...
%>
						...
						<li <%if (cnt ==1) { %> class="active" <%} %>><a href="#"><span class="newTitle"><%=cnt %></span></a>
							<ul>
								<li>
									<span class="img"><a class="view_page" href="../video/video_player.jsp?ocode=<%=oinfo.getOcode()%>"><span class="playSmall"></span><img src="<%=imgTmp %>" alt="<%=oinfo.getTitle()%>"/></a></span>
									<span class="total">
										<a class="view_page" href="../video/video_player.jsp?ocode=<%=oinfo.getOcode()%>"><%=oinfo.getTitle()%></a>
									</span>
								</li>
							</ul>
						</li>
				...
				<div class="mNote">
					...
					if (noticeVt2 != null && noticeVt2.size() > 0) {
					 ...
					 %>
					 	...
					 	<span class="img"><a class="view_page" href="../board/board_view.jsp?board_id=<%=bliBean.getBoard_id() %>&amp;list_id=<%=bliBean.getList_id()%>"><img src="../board/img_2.jsp?board_id=<%=bliBean.getBoard_id() %>&amp;list_id=<%=bliBean.getList_id()%>" alt="제작현장"/></a></span>
						<span class="total">
				...
				<div id="popupzone">
					...
					%>							
						...
						<div class="controller" id="popbtn">
							<ul>
								<li><a href="#popupZone" onclick="showHide('popupList');return false;"><img src="../include/images/popup_list.gif" alt="정지" /></a>
...
%>
				 ...
				 <%if (cnt ==1) { %>
				<li class="big">
					<span class="img"><a class="view_page" href="../video/video_player.jsp?ocode=<%=oinfo.getOcode()%>"><span class="playMid"></span><img src="<%=imgTmp %>" alt="<%=oinfo.getTitle()%>"/></a></span>
					<span class="total">
				...
				<%} else { %>
				<li class="small">
					<span class="img"><a class="view_page" href="../video/video_player.jsp?ocode=<%=oinfo.getOcode()%>"><span class="playSmall2"></span><img src="<%=imgTmp %>" alt="<%=oinfo.getTitle()%>"/></a></span>
				</li>
				<%} %>
...
%>				
			...
			<div class="live">
				...
				<%if (i == 1) { %>				
				<li class="big">
					<span class="img"><a class="view_page" href="../video/video_player.jsp?ccode=<%=oinfo.getCcode()%>&amp;ocode=<%=oinfo.getOcode()%>"><span class="playMid"></span><img src="<%=imgTmp%>" alt="<%=oinfo.getTitle()%>"/></a></span>
					<span class="total">
				...
				<%} else { %>
				<li class="small">
					<span class="img"><a class="view_page" href="../video/video_player.jsp?ccode=<%=oinfo.getCcode()%>&amp;ocode=<%=oinfo.getOcode()%>"><span class="playSmall2"></span><img src="<%=imgTmp%>" alt="<%=oinfo.getTitle()%>"/></a></span>
				</li>
				<%} %>
