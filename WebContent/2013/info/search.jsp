...
Hashtable result_ht = null; 
 ...
 if(request.getParameter("board_id") != null && request.getParameter("board_id").length() > 0 && !request.getParameter("board_id").equals("null") && com.yundara.util.TextUtil.isNumeric(request.getParameter("board_id")))
 {
	...
	if(request.getParameter("search_ccode") != null && request.getParameter("search_ccode").length() > 0 && !request.getParameter("search_ccode").equals("null")){
		search_ccode = request.getParameter("search_ccode");
	}
	String search_ycode = "";
...
if ( (searchString != null && searchString.length() > 0) || (search_ycode != null && search_ycode.length() > 0) || (xcode != null && xcode.length() > 0) ) {
					...
					<input type="hidden" name="search_ccode" value="<%=search_ccode%>" />
						...
						<legend>영상검색</legend>
							<div class="searchTop">
								...
								<option value="all" <% if (searchField != null &&  searchField.equals("all") ) {out.println(" selected='selected' ");}%>>제목+내용</option>
								</select>
								<input type="text" title="검색어" name="searchString" value="<%=searchString %>" class="input_text"/> 
								<input type="image" src="../include/images/btn_search.gif" alt="검색"/>
							</div>
							<dl>
...
%>
				<li class="pro"><a href="/2013/info/search.jsp?search_ycode=<%=info.getCcode()%>"><%=info.getCtitle()%></a>
				<ul>
...
<%
	...
	%>
						...
						<li><a href="/2013/info/search.jsp?search_ycode=<%=info.getCcode()%>"><%=info.getCtitle()%></a></li>
...
%>					
						<li>
							<span class="img"><a class="view_page" href="../video/video_player.jsp?ocode=<%=oinfo.getOcode()%>"><img src="<%=imgTmp %>" alt="<%=strTitle %>"/></a></span>
							<span class="info">
								<span class="day"><%=wdate %></span>
								<span class="cate1"><%=temp_ctitle %> </span>
								<span class="title"><a class="view_page" href="../video/video_player.jsp?ocode=<%=oinfo.getOcode()%>"><%=oinfo.getTitle() %></a></span>
