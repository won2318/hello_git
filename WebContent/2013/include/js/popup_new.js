	...
	{ 
		if(obj_a = document.getElementById("popup_a"+i))
		{
			...
			if(i==value) {
				...
				if(imgobj_a = p) imgobj_a.src = imgobj_a.src.replace("off.gif","on.gif"); //  숫자 이미지 명의 off를 on으로 바꾸어 이미지의 변환
			} else {
				obj_a.style.display="none"; // i와 value값이 다를 경우 이미지 none 처리
				if(imgobj_a = p) imgobj_a.src = imgobj_a.src.replace("on.gif","off.gif"); //on 이미지를 off 이미지로 변환
			}
		}
	}
}
...
function play_popup_a(value) 
{
	...
	{ 
		...
		setTimeout("play_popup_a()", 100000); // 일정시간후 play_popup 한번실행 (setinterval의 경우 반복실행이기에 오류가 남)
	}
}
	...
	{ 
		if(obj_b = document.getElementById("popup_b"+i))
		{
			...
			if(i==value) {
				...
				if(imgobj_b = p) imgobj_b.src = imgobj_b.src.replace("off.gif","on.gif"); //  숫자 이미지 명의 off를 on으로 바꾸어 이미지의 변환
			} else {
				obj_b.style.display="none"; // i와 value값이 다를 경우 이미지 none 처리
				if(imgobj_b = p) imgobj_b.src = imgobj_b.src.replace("on.gif","off.gif"); //on 이미지를 off 이미지로 변환
			}
		}
	}
}
...
function play_popup_b(value) 
{
	...
	{ 
		...
		setTimeout("play_popup_b()", 100000); // 일정시간후 play_popup 한번실행 (setinterval의 경우 반복실행이기에 오류가 남)
	}
}
	...
	{ 
		if(obj_c = document.getElementById("popup_c"+i))
		{
			...
			if(i==value) {
				...
				if(imgobj_c = p) imgobj_c.src = imgobj_c.src.replace("off.gif","on.gif"); //  숫자 이미지 명의 off를 on으로 바꾸어 이미지의 변환
			} else {
				obj_c.style.display="none"; // i와 value값이 다를 경우 이미지 none 처리
				if(imgobj_c = p) imgobj_c.src = imgobj_c.src.replace("on.gif","off.gif"); //on 이미지를 off 이미지로 변환
			}
		}
	}
}
...
function play_popup_c(value) 
{
	...
	{ 
		...
		setTimeout("play_popup_c()", 100000); // 일정시간후 play_popup 한번실행 (setinterval의 경우 반복실행이기에 오류가 남)
	}
}
	...
	{ 
		if(obj_d = document.getElementById("popup_d"+i))
		{
			...
			if(i==value) {
				...
				if(imgobj_d = p) imgobj_d.src = imgobj_d.src.replace("off.gif","on.gif"); //  숫자 이미지 명의 off를 on으로 바꾸어 이미지의 변환
			} else {
				obj_d.style.display="none"; // i와 value값이 다를 경우 이미지 none 처리
				if(imgobj_d = p) imgobj_d.src = imgobj_d.src.replace("on.gif","off.gif"); //on 이미지를 off 이미지로 변환
			}
		}
	}
}
...
function play_popup_d(value) 
{
	...
	{ 
		...
		setTimeout("play_popup_d()", 100000); // 일정시간후 play_popup 한번실행 (setinterval의 경우 반복실행이기에 오류가 남)
	}
}
	...
	{ 
		if(obj_e = document.getElementById("popup_e"+i))
		{
			...
			if(i==value) {
				...
				if(imgobj_e = p) imgobj_e.src = imgobj_e.src.replace("off.gif","on.gif"); //  숫자 이미지 명의 off를 on으로 바꾸어 이미지의 변환
			} else {
				obj_e.style.display="none"; // i와 value값이 다를 경우 이미지 none 처리
				if(imgobj_e = p) imgobj_e.src = imgobj_e.src.replace("on.gif","off.gif"); //on 이미지를 off 이미지로 변환
			}
		}
	}
}
...
function play_popup_e(value) 
{
	...
	{ 
		...
		setTimeout("play_popup_e()", 100000); // 일정시간후 play_popup 한번실행 (setinterval의 경우 반복실행이기에 오류가 남)
	}
}
