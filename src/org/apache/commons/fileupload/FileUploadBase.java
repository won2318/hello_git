...
public abstract class FileUploadBase {
 ...
 System.out.println("parseRequest:");       
            while (iter.hasNext()) {
                ...
                //today time 
                SimpleDateFormat fommatter = new SimpleDateFormat("yyyyMMddHHmmssSSS");
                ...
                if (fileName != null && fileName.length() > 0 && fileName.indexOf(".") > 0) {
                	temp_fName = str_result + fileName.substring(fileName.lastIndexOf("."),fileName.length()); 
//                	temp_fName = str_result + fileName;
//                	com.hmkcode.vo.FileMeta fmeta = new com.hmkcode.vo.FileMeta();
//                  	fmeta.setImg_title(fileName);                 	
                }  
 
 
//System.out.println("FileUploadBase-->parseRequest:"+item.getFieldName());

                FileItem fileItem = fac.createItem(item.getFieldName(), item.getContentType(),
                                                   item.isFormField(), temp_fName );
                items.add(fileItem);
