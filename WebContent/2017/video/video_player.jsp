	...
	if (vo != null && vo.size() > 0) {
		try {
			...
			if (omiBean.getThumbnail_file() != null && omiBean.getThumbnail_file().indexOf(".") > 0) {
				imgTmpThumb = "http://tv.suwon.go.kr/upload/vod_file/"+omiBean.getThumbnail_file();
			}
 ...
 // // 카카오링크 버튼을 생성합니다. 처음 한번만 호출하면 됩니다.
    function sendLink() {
        Kakao.Link.sendDefault({
          ...
          addressTitle: '수원시 인터넷 방송',
          content: {
            title: '<%=otitle.replaceAll("&#39;","").replaceAll("'"," ").replaceAll("[\r\n]", " ")%>',
            description: '<%=otitle.replaceAll("&#39;","").replaceAll("'"," ").replaceAll("[\r\n]", " ")%>',
            imageUrl: '<%=imgTmpThumb %>',
            link: {
...
일부 프로그램이 원활하게 작동 하지 않을수 있습니다!<br/> 
			...
			<% if (ccode.contains("004001")) { %> 
			 		<div class="btn_write_ab i_write"><a href="javascript:boardWrite('11');">등록</a></div>
			<%} else if (ccode.contains("004002")) { %>
			 		<div class="btn_write_ab i_write"><a href="javascript:boardWrite('13');">등록</a></div>
			<%} else if (ccode.contains("004003")) { %>
			 		<div class="btn_write_ab i_write"><a href="javascript:boardWrite('19');">등록</a></div>
			<% }%>
					...
					<div class="vodView">
						<div class="data">
							...
							<div class="info">
								...
								<i class="i_play"><%=oHit_count%></i>
								<i class="i_reply"><a href="#comment_inner"><%if(memo_ht != null && !memo_ht.isEmpty() && memo_ht.size() >= 1 && (memoVt != null && memoVt.size()>0)){%><%=mPageBean.getTotalRecord()%><%}else{%>0<%}%></a></i>
								<i class="i_like"><a href="#like_boom"><%=recomcount%></a></i>
							</div>
							<span class="sns">
								<h5>퍼가기</h5>
				 
								<a href="javascript:copy_select();"><img src="../include/images/sns_url_s.gif" alt="url"/></a>
								<a href="javascript:iframe_select();"><img src="../include/images/sns_iframe_s.gif" alt="iframe"/></a>
								<a href="javascript:postToFeed();"><img src="../include/images/sns_facebook_s.gif" alt="facebook"/></a>
								<a href="javascript:twitter_share();"><img src="../include/images/sns_twitter_s.gif" alt="twitter"/></a>
								<a href="javascript:shareStory()"><img src="../include/images/sns_cacaopage_s.gif" alt="cacao story"/></a>
<!-- 								<a href="javascript:sendLink()"><img src="../include/images/sns_cacaopage_s.gif" alt="cacao Link"/></a> -->
								<a href="javascript:naverBand()"><img src="../include/images/sns_band_s.gif" alt="naver band"/></a>
							</span>
						</div>
...
<%-- 								  <iframe title="동영상 재생 창" id="video_player" name="video_player" src="/videoJs_2019/jsPlayer.jsp?ocode=<%=ocode%>&type=sub" scrolling="no" width="690" height="388" marginwidth="0" frameborder="0" framespacing="0" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true" oallowfullscreen="true" msallowfullscreen="true"></iframe> --%>
								  <iframe title="동영상 재생 창" id="video_player" name="video_player" src="/videoJs/jsPlayer_2017.jsp?ocode=<%=ocode%>&type=sub" scrolling="no" width="690" height="388" marginwidth="0" frameborder="0" framespacing="0" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true" oallowfullscreen="true" msallowfullscreen="true"></iframe>
								  <%} else { %>
								  	등록된 정보가 없습니다.
								  <%} %>
						...
						<div class="data2">
							<span class="subj clearfix"><%=chb.getContent_2(odesc,"true")%></span>
							<div class="like_boom" id="like_boom">
								<a href="javascript:point_go();"><span id="recomcount"><%=recomcount%></span></a>
							</div>
						</div>
						<!--//data-->
						...
						<div class="newsData"> 
							 ...
							 <% if (news != null) {
							 	...
							 	%>
									<dl>
										<dt class="title"><a href="<%=newsInfoBean.getLink()%>"><%=newsInfoBean.getTitle()%></a><span class="day"><%=newsInfoBean.getWdate() %></span></dt> 
									</dl> 
								<%}} %>
							</div>
						</div>-->
