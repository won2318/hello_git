...
public class SEEDUtil{
	...
	private static final String DEFAULT_IV = com.vodcaster.sqlbean.DirectoryNameManager.SEED_SECURITY;
...
//			KC0 = golden ratio; KCi = ROTL(KCi-1, 1) 	
	...
	// reorder for little endian	  
		...
		public static String getEncrypt(String encVal) {
			String value="";
		 	
			try {
				value =  getSeedEncrypt(TextUtil.getValue(encVal),  getSeedRoundKey(com.vodcaster.sqlbean.DirectoryNameManager.SEED_SECURITY));
			} catch (Exception e) {
		...
		public static String getDecrypt(String encVal) {
			String value="";
		 	
			if (encVal != null && encVal.length() > 0) {
			try {
				value =  getSeedDecrypt(TextUtil.getValue(encVal),  getSeedRoundKey(com.vodcaster.sqlbean.DirectoryNameManager.SEED_SECURITY));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
